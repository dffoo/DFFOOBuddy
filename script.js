...
function loadDefaultBanner(value) {
    document.getElementById('img-banner').src = "img/banner/"+value+".png";
    selectedBannerValues = getBannerValues(value);
    updateAutopullLabels();
}

function selectBanner(value) {
    var filename = `img/banner/${value}.png`;
    ...
    // special logic for no-ld banner
    if(value == 346) {
        bannerHasNoLD = true;
        document.getElementById('check-ld').style.display = "none";
        document.getElementById('label-ld').style.display = "none";
        document.getElementById('ld-spacer').style.display = "none";
        document.getElementById("check-ld").checked = false;
    } else {
        bannerHasNoLD = false;
        document.getElementById('check-ld').style.display = "unset";
        document.getElementById('label-ld').style.display = "unset";
        document.getElementById('ld-spacer').style.display = "unset";
    }
}
...
function pullSingle(isAutoPull) {
    ...
    if((bannerHasNoLD) && (luckyNumber >= 9941 && luckyNumber <= 9990)){ // if banner has no LD, reroll LD to any EX
        // console.log("ld: " + luckyNumber);
        luckyNumber = Math.floor(Math.random() * (9940 - 9716 + 1) + 9716); // 9716-9940 (reroll to any ex)
    }
...
function generateMultiPull() {
    ...
    if((bannerHasNoLD) && (luckyNumberBonus >= 9201 && luckyNumberBonus <= 9700)){ // if banner has no LD, reroll LD to any EX
        // console.log("ld bonus: " + luckyNumberBonus);
        luckyNumberBonus = Math.floor(Math.random() * (9200 - 6951 + 1) + 6951); // 6951-9200 (reroll to any ex)
    }
...
function generateGoldWeapon(i, weaponId, labelId, counter) {
    document.getElementById(weaponId).src = "img/eq/loading.png";
    if(i < 9132) {  // 8990-9131 = 1.42%
